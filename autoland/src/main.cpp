...
class ImageConverter
{
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		//	for ( int i = 1; i < 30; i = i + 2 )
		  //      	GaussianBlur( frame_hsv, frame_hsv_1 , Size( i, i ), 0, 0 );
		...
		//	vector<vector<Point> > hull_A( contours_A.size() );
		//	for( size_t i = 0; i < contours_A.size(); i++ )
		//		{  convexHull( Mat(contours_A[i]), hull_A[i], false ); }
				
		//	vector<vector<Point> > hull_B( contours_B.size() );
		//	for( size_t i = 0; i < contours_B.size(); i++ )
		//		{  convexHull( Mat(contours_B[i]), hull_B[i], false ); }	
			...
			//	drawContours( frame, hull_A, max_area_index, color, 2, 8, vector<Vec4i>(), 0, Point() );
			...
			//	drawContours( frame, hull_B, max_area_index, color, 2, 8, vector<Vec4i>(), 0, Point() );
			...
			//imshow(WINDOW2, frame_hsv_1);
			//imshow(WINDOW, frame_hsv);
			waitKey(3);
