...
class KeyMapping(object):
	...
	SwapCamera       = QtCore.Qt.Key.Key_C
...
class KeyboardController(DroneVideoDisplay):
	...
	def ReceiveVispPosition(self,position):
		...
		vispPosition = position
		#rospy.loginfo(position)
		
		if self.vispStatus == 3:
			rospy.loginfo("!!!!! Automatically moving drone!")
			self.vispStatusActioned = self.vispStatus
			self.pitch = 0
			self.roll = 0#-position.pose.position.x
			self.yaw_velocity = 0
			self.z_velocity = 2 * position.pose.position.y
			controller.SetCommand(self.roll, self.pitch, self.yaw_velocity, self.z_velocity)
		
		elif self.vispStatusActioned != self.vispStatus: 
			rospy.loginfo("!!!!! Stop automatically moving drone!")
			self.vispStatusActioned = self.vispStatus
			self.pitch = 0
			self.roll = 0
			self.yaw_velocity = 0
	...
	def keyPressEvent(self, event):
		...
		if controller is not None and not event.isAutoRepeat():
			...
			if key == KeyMapping.Emergency:
				...
				controller.SendLand()
			elif key == KeyMapping.SwapCamera:
				controller.SendLand()
			else:
...
if __name__=='__main__':
	...
	rospy.loginfo("!!!!!!!!!!!!!!!!!!!!!!!AUTOLAND: Starting autolanding function")
